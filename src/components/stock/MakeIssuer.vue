<template>
    ...
    <header class="fixed-top" style="position: fixed;" >
     ...
     <nav  style="margin-right: 90px;white-space: nowrap;color: white;margin-top: 15px;" id="sidebar">
       <ul class="fixed">
         <li>
           ...
           <ul>
             ...
             <li class="dropDown-menu fixed-top">
               ...
               <ul>
                 ...
                 <li><a href="/makeIssuer" style="font-size: 16px;font-family:inter;font-weight:medium">Make Issuer</a></li>
   ...
   <div class="form-control">
    ...
    <input type="text" v-model="search" id="table-search" placeholder="Search...">
       ...
       <span v-if="user.issuer" style="cursor: pointer; color: red; text-decoration: underline;" @click.prevent="RemoveUserApprover(user.email)">Remove Issuer</span>
     </td>
             </tr>
           </tbody>
         </table>
 ...
 .issuer-row {
   background-color:  rgb(182, 209, 250); /* Change the background color to red */
 ...
 export default {
  ...
  filteredBrands() {
   ...
   if (this.selectedUsers.includes(user)) {
     ...
     makeSelectedUsersApprovers() {
       ...
       console.log("response on email confirmation: ", response);
             if (response.code == "200") {
               ...
               user.issuer=true; // Mark the user as an approver
             }
           }
         })
       ).then(() => {
         swal.fire({ html: `<p class="text-success">Selected users have been made issuers.</p>` });
     ...
     async MakeUserApprover(useremail) {
       var response = await this.MakeIssuer(useremail);
       console.log("response on email confirmation: ", response);
       if (response.code == "200") {
         swal.fire({ html: `<p class="text-success">${response.message}</p>` });
         this.getallusers();
  ...
  var response = await this.RemoveIssuer(useremail);
  console.log("response on email confirmation: ", response);
  if (response.code == "200") {
    swal.fire({ html: `<p class="text-success">${response.message}</p>` });
