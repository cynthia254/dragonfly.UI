        ...
        <div class="col">
          <nav aria-label="breadcrumb" class="bg-light rounded-3 p-3 mb-4">
            <ol class="breadcrumb mb-0">
              ...
              >
             List of all issued items in Month {{ this.month }}/{{ this.year }}
              </li>
            </ol>
          </nav>
        </div>
      ...
      <div class="table-wrapper">
        ...
        >
          <div class="row">
            <div class="col-sm-6">
              <h2
                ...
                "
              >
               LIST OF ITEMS ISSUED IN MONTH {{ this.month }}/{{ this.year }}
              </h2>
            </div>
          </div>
          ...
          <tr>
            ...
            <tbody >
              <tr
                v-for="(invoice, index) in this.batchbody"
                :key="invoice.id"
                style="
                          font-family: inter;
                          font-size: 16px;
        ...
        <span
          v-if="invoice.categoryName !== 'Accesory'"
          @click="ModalOpen(invoice)"
        ...
        <span v-else>
            {{ invoice.brandName }} {{ invoice.itemName }}
                ...
                <td     >{{ invoice.quantityDispatched }}</td>
                <td     >{{ invoice.nameToUse }}</td>
  ...
  // Assuming batchID is defined and contains the correct value
     ...
     //this.reference_number=this.invoiceItemBody.reference_Number;
      ...
      getStatusStyle(invoice) {
        if (invoice.productStatus === "Complete") {
          return {
            color: "green",
          };
        } else if (invoice.productStatus === "Incomplete") {
          return {
            color: "red",
          };
        } else if (invoice.productStatus === "Pending") {
          return {
            color: "orange",
          };
        } else if (invoice.productStatus === "Closed") {
          return {
            color: "blue",
          };
        } else {
      ...
      async editinvoiceitem(invoice) {
        if (invoice.categoryName !== "Accesory") {
          console.log("Navigating to edit page for:", invoice);
          this.$router.push({
            path: `/invoice_item/${invoice.batchNumber}`,
            replace: true,
          });
        }
      },
      ...
      async gettingproductdetailsbyid() {
        // Assuming batchID is defined and contains the correct value
        var response = await this.GetAllItemsIssuedPerMonth(this.year,this.month);
      ...
      async ModalOpen(invoice) {
    console.log("ModalOpen called with", invoice);
    this.selectedBatchNumber = invoice.id;
    ...
    var response = await this.GetSerialIssued(invoice.id);
