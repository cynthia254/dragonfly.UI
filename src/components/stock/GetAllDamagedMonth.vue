<template>
        ...
        <div class="col">
          <nav aria-label="breadcrumb" class="bg-light rounded-3 p-3 mb-4">
            <ol class="breadcrumb mb-0">
              ...
              >
                List of Damaged Items in Month {{ this.month }}/{{ this.year }}
              </li>
            </ol>
          </nav>
        </div>
          ...
          <div class="row">
            <div class="col-sm-6">
              <h2
                ...
                "
              >
              LIST OF ALL ITEMS DAMAGED IN MONTH  {{this.month}}/{{this.year}}
              </h2>
            </div>
          </div>
  ...
  <div v-for="(serialNumber, index) in selectedSerialNumbers" :key="index" style="font-family: inter; font-size: 16px; font-weight: medium; color: gray;">
    <p>
      ...
      <span style="font-weight: bold; color: black;">Condition Status:</span> {{ serialNumber.itemStatus }}<br>
    </p>
  </div>
</div>
    </div>
  ...
  // Assuming batchID is defined and contains the correct value
     ...
     //this.reference_number=this.invoiceItemBody.reference_Number;
      ...
      async gettingproductdetailsbyid() {
        // Assuming batchID is defined and contains the correct value
        var response = await this.GetAllItemsDamagedParticularMonth(this.year,this.month);
  ...
  // Use an API call or a method to retrieve "Faulty" serial numbers for the selected batch
  // Replace the following line with your actual data retrieval logic
  var response = await this.getFualtySerial(invoiceitem.batchNumber);
  this.selectedSerialNumbers = response.body;

  console.log("Filtered serial numbers: ", this.selectedSerialNumbers);
