 ...
 export default {
   ...
   methods:{
     async gettingsalesbyid() {
       ...
       var response = await this.gettingBrandById(brandid);
       this.brandBody = response.body;
       console.log("response on brand body: : ", this.brandBody);
 ...
 formatDateAssigned(value) {
     ...
     async changepurchasestatus(){
       var body = {
         ...
         brandName:this.brandBody.brandName,
       ...
       console.log("user id: ", body)
            var resp= await this.edititngBrand(body);
            if(resp.isTrue == true){
